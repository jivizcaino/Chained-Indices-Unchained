...
function sim_model_full(params::Vector{Float64};Pg_t::Vector{Float64},Ps_t::Vector{Float64},
    ...
    for t in 1:(2024-1980) 
        ...
        # R = θ calAx K^(θ-1) L^(1-θ) = θ calAxhat k^(θ-1) with L = N h, which is
        # the form used for the Euler equation and for dedt_e below.
        push!(eq["Rt"],θ*eq["calAxhat"][t]*eq["kt"][t]^(θ-1))
        push!(eq["Wt"],(1-θ)*eq["calAxhat"][t]*eq["kt"][t]^θ)

        # Xst and Xgt are expenditure values, since Xgt = Xt - Xst, so their ratio
        # is already a value ratio and must not be multiplied by prices again.
        # Cgt and Cst are quantities, so the consumption term does carry prices.
        Ls = (eq["Xt"][t]/eq["Yt"][t])*(1/((eq["Xgt"][t]/eq["Xst"][t])+1))+(eq["Et"][t]/eq["Yt"][t])*(1/(((eq["Pgt"][t]*eq["Cgt"][t])/(eq["Pst"][t]*eq["Cst"][t]))+1))
...
if run_smm
    ...
    function SSE(params;cons_exp_share_data)
        ...
        if any(x -> isnan(x) || isinf(x), sim_eq["sgt"])
            ...
            return 9e12
        else SSE =  ( sum(  (sim_eq["sgt"] .- cons_exp_share_data).^2 ) ) #Sum of Squared Errors 
            #@show SSE
            return SSE
        end
    end
    ...
    for seed in seeds
        ...
        opt_problem = bbsetup(params -> SSE(params;cons_exp_share_data=VAC_GOOD_SHARE);
                            ...
                            PopulationSize          = 100,
                            MaxFuncEvals            = 200_000,
                            FitnessTolerance        = 1e-10,
...
elseif run_smm_fixed_chi
    ...
    function SSE_fixed_chi(params; cons_exp_share_data)
        ...
        if any(x -> isnan(x) || isinf(x), sim_eq["sgt"])
            ...
            return ( sum( (sim_eq["sgt"] .- cons_exp_share_data).^2 ) )
        end
    end
...
#Sanity Check: the deviation term vanishes at the base year
...
FS_2023_2023_τ   = chain_index([0.0;gD_2023_2023_τ])
...
FS_2010_2010_τ   = chain_index([0.0;gD_2010_2010_τ])
...
FS_2000_2000_τ   = chain_index([0.0;gD_2000_2000_τ])
...
FS_1990_1990_τ   = chain_index([0.0;gD_1990_1990_τ])
...
FS_1980_1980_τ   = chain_index([0.0;gD_1980_1980_τ])
...
FS_1980_h_h        = chain_index([0.0;gD_1980_h_h])
...
FS_2023_h_h      = chain_index([0.0;gD_2023_h_h])
...
FS_t_1980_τ     = chain_index([0.0;gD_t_1980_τ])
...
FS_t_2023_τ      = chain_index([0.0;gD_t_2023_τ])
...
#
# Notation. In m̂_{t,z,τ} the first index is the base preference order, the second
# the base prices and the third the date evaluated. A fixed-base index sets t = z
# at the base year and lets τ run, so the running index here is τ, not z.
#
...
# the equivalent variation measure of income, not actual income. At its own base
# ê_{t,t} = e_t and m̂_{t,t,t} = m_t, so the ratio starts at s_{e,t} and, for the
# current-base index, declines in τ converging to s_{e,t1} from above.
#
# mhat_1980_1980_τ and mhat_2023_2023_τ are the fixed-base series already built
# for Figure 5(a), so they are reused here.

# At preferences and prices of 1980
t_prime            = 1980 - 1980 + 1
e_1980_1980        = e_t_x(t_prime, t_prime; χ=χ, ν_t=ν_t, Pst=sim_eq["Pst"])
se_1980_τ          = e_1980_1980 ./ mhat_1980_1980_τ

# At preferences and prices of 2023
t_prime            = 2023 - 1980 + 1
e_2023_2023        = e_t_x(t_prime, t_prime; χ=χ, ν_t=ν_t, Pst=sim_eq["Pst"])
se_2023_τ          = e_2023_2023 ./ mhat_2023_2023_τ

se                 = sim_eq["et"]./sim_eq["yt"]

### At its own base the hypothetical objects collapse onto the actual ones,
### since B_{t,t,t} = 0 and ê_{t,t} = e_t.
@assert isapprox(e_1980_1980, sim_eq["et"][1];   rtol=1e-8)
@assert isapprox(e_2023_2023, sim_eq["et"][end]; rtol=1e-8)
@assert isapprox(se_1980_τ[1],   se[1];   rtol=1e-8)
@assert isapprox(se_2023_τ[end], se[end]; rtol=1e-8)

### Appendix D: the current-base share declines in τ, converging from above to s_e.
@assert all(diff(Float64.(se_2023_τ)) .< 0)

### Axis limits from the three series, so the panel is not compressed against a
### fixed range. Rounded outwards to the tick step.
se_all             = Float64.(vcat(se, se_1980_τ, se_2023_τ))
se_step            = 0.25
se_lo              = 0.25
se_hi              = 1.25
...
# EV goods expenditure share. Appendix D, equation (41): applying Roy\'s identity
# to V(ê_{t,z}, P_{g,z}, P_{s,z}) gives an object independent of the date τ, and
# for a fixed base t = z it collapses to ŝ_{g,t,t} = s_{g,t}. The two lines are
# therefore horizontal in τ by construction.
t_prime    = 2023 - 1980 + 1
sg_2023_τ  = sg_t_x(t_prime, t_prime; χ=χ, η=η, γ=γ, Pst=sim_eq["Pst"], Pgt=sim_eq["Pgt"])

t_prime    = 1980 - 1980 + 1
sg_1980_τ  = sg_t_x(t_prime, t_prime; χ=χ, η=η, γ=γ, Pst=sim_eq["Pst"], Pgt=sim_eq["Pgt"])

@assert isapprox(sg_1980_τ, sim_eq["sgt"][1];   rtol=1e-8)
@assert isapprox(sg_2023_τ, sim_eq["sgt"][end]; rtol=1e-8)

plot(1980:2023, sim_eq["sgt"], label=L"s_{g,\tau}",linestyle=:solid, lw=2,color=:black)
...
πD  = se_w.*( sg_w.*πg .+ (1 .- sg_w).*πs )
(πg - πs)[end] 
s_e[end]
100*(πD[1] - πD[end])

#Alternative Decomposition of the Decline of the Growth Rate
#x_t, s_e, g_cg, g_cs, g_x and gD are those built for Figure 4 (a).
gD[end] - gD[1]
#------------------------------------------------------------------------

#------------------------------------------------------------------------
#APPENDIX A.5
# Quantitative Evaluation of Condition (S_g,t0) in Step 1
...
#------------------------------------------------------------------------
# Quantitative Evaluation of Condition (S_g,t1) in Step 1
...
#
# Fixed-base Fisher-Shell indices for consumption expenditure are built from the
# CONSUMPTION-ONLY Konus expenditure function
#
#     etilde(t,z) = E( V(e_z, Pg_z, Ps_z), Pg_t, Ps_t )
#                 = [ (e_z/Ps_z)^χ + (ηχ/γ)( (Pg_t/Ps_t)^γ - (Pg_z/Ps_z)^γ ) ]^(1/χ) * Ps_t
#
# notice that it contains NO marginal value of capital. The growth rate of the fixed-base
# index with base t is
#
#     g_e(t,z) = gD_e(z) * ( e_z/etilde(t,z) * Ps_t/Ps_z )^χ
#              = gD_e(z) / [ 1 + (χ/γ) * sg_z * ( (p_t/p_z)^γ - 1 ) ],   p = Pg/Ps
#
#------------------------------------------------------------------------

#--- Helper functions ---------------------------------------------------
# relative price of goods
p_rel(x; Pst, Pgt) = Pgt[x] / Pst[x]

# consumption-only Konus expenditure: base-t prices, period-z utility
etilde_t_z(t, z; χ, η, γ, Pst, Pgt, et) =
    ( (et[z]/Pst[z])^χ +
      (η*χ/γ)*( p_rel(t; Pst=Pst, Pgt=Pgt)^γ - p_rel(z; Pst=Pst, Pgt=Pgt)^γ ) )^(1/χ) * Pst[t]

# correction factor, closed form (equivalent to the ratio built from etilde_t_z)
aux_e_cons(t, z; χ, γ, Pst, Pgt, sgt) =
    1 / ( 1 + (χ/γ)*sgt[z]*( ( p_rel(t; Pst=Pst, Pgt=Pgt) / p_rel(z; Pst=Pst, Pgt=Pgt) )^γ - 1 ) )

# correction factor, direct form (used only as a numerical cross-check)
aux_e_cons_direct(t, z; χ, η, γ, Pst, Pgt, et) =
    ( ( et[z] / etilde_t_z(t, z; χ=χ, η=η, γ=γ, Pst=Pst, Pgt=Pgt, et=et) ) * (Pst[t]/Pst[z]) )^χ
#------------------------------------------------------------------------

#--- Chained Divisia ----------------------------------------------------
# Tornqvist discretisation: log growth rates with midpoint share weights,
# cumulated with chain_index. The same construction is used in LV_main_NHCES.jl
# so that the gaps are directly comparable across preference specifications.
cgt_f      = Float64.(sim_eq["cgt"])
cst_f      = Float64.(sim_eq["cst"])
sg_w_e     = chain_share(Float64.(sim_eq["sgt"]))
...
# The correction factor is averaged over each interval to match the midpoint
# share weights, exactly as in LV_main_NHCES.jl.
aux_mid_e(t::Int, z::Int) =
    0.5 * (aux_e_cons(t, z-1; χ=χ, γ=γ, Pst=sim_eq["Pst"], Pgt=sim_eq["Pgt"], sgt=sim_eq["sgt"]) +
           aux_e_cons(t, z;   χ=χ, γ=γ, Pst=sim_eq["Pst"], Pgt=sim_eq["Pgt"], sgt=sim_eq["sgt"]))

g_e_cons(t::Int) = gD_e_z .* [aux_mid_e(t, z) for z in 2:length(z_prime)]
...
g_e_2023_z = g_e_cons(t_prime)
P_e_2023_z = chain_index([0.0; g_e_2023_z .+ g_n])

#Base 2010
t_base     = 2010
t_prime    = t_base - 1980 + 1

g_e_2010_z = g_e_cons(t_prime)
P_e_2010_z = chain_index([0.0; g_e_2010_z .+ g_n])

#Base 2000
t_base     = 2000
t_prime    = t_base - 1980 + 1

g_e_2000_z = g_e_cons(t_prime)
P_e_2000_z = chain_index([0.0; g_e_2000_z .+ g_n])

#Base 1990
t_base     = 1990
t_prime    = t_base - 1980 + 1

g_e_1990_z = g_e_cons(t_prime)
P_e_1990_z = chain_index([0.0; g_e_1990_z .+ g_n])

#Base 1980
t_base     = 1980
t_prime    = t_base - 1980 + 1

g_e_1980_z = g_e_cons(t_prime)
L_e_1980_z = chain_index([0.0; g_e_1980_z .+ g_n])
#---------------------------------------------------------------
...
let Pst = sim_eq["Pst"], Pgt = sim_eq["Pgt"], sgt = sim_eq["sgt"], et = sim_eq["et"]
    ...
    err_dir = maximum([ abs(aux_e_cons(t, z; χ=χ, γ=γ, Pst=Pst, Pgt=Pgt, sgt=sgt) -
                            aux_e_cons_direct(t, z; χ=χ, η=η, γ=γ, Pst=Pst, Pgt=Pgt, et=et))
                        for t in bases, z in z_prime ])
    println("""
    Appendix C checks
      max |factor(t,t) - 1|              : $(round(err_one, sigdigits=3))
      max |closed form - direct form|    : $(round(err_dir, sigdigits=3))
      P_e(2023) - D_e  (cumulative)      : $(round(P_e_2023_z[end] - D_e_z[end], digits=4))
      L_e(1980) - D_e  (cumulative)      : $(round(L_e_1980_z[end] - D_e_z[end], digits=4))
    """)
end

# With the income-elastic good (services) being the one whose relative price rises, the
...
consumption_index_table = DataFrame(
    Year      = collect(1980:2023),
    D_e_z      = D_e_z .- D_e_z,
    P_e_2023_z = P_e_2023_z .- D_e_z,
    L_e_1980_z = L_e_1980_z .- D_e_z,
)
pretty_table(consumption_index_table, alignment=:l)

io = IOBuffer()
println(io, """
$(repeat("=", 60))
Real Consumption Expenditure Indices (1980–2023)
$(repeat("=", 60))
""")
pretty_table(io, consumption_index_table, alignment=:l, crop=:none)
println(io, "$(repeat("=", 60))\n")
...
plot!(1981:2023, gD_e_z, 
    ...
    framestyle=:box)
if save_figures
    savefig(joinpath(figuresdir, "growth_consumption_expenditure_indices_bases.png"))
    println("Figure saved to: ", joinpath(figuresdir, "growth_consumption_expenditure_indices_bases.png"))
end
#---------------------------------------------------------------
